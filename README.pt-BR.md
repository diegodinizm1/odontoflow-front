...
- 🛡️ **Acesso por papel** — recepcionistas não veem Financeiro, Equipe nem Assinatura (ocultos no menu e bloqueados por guard de rota); têm uma tela de **Cobranças** para criar e quitar cobranças. Um menu de usuário na sidebar mostra nome + função e abre **perfil** (trocar senha) e **configurações** (clínica + link de agendamento).
...
- 🗂️ **Planos de tratamento** — monte um orçamento com itens no prontuário; conclua itens para faturá-los no financeiro.
...
```bash
npm test          # testes unitários (Angular + Vitest) em modo watch
npm run test:ci   # testes unitários, execução única (usado na CI)
